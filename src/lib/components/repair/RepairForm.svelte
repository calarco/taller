...
<Form action="?/upsertRepair" --grid-columns="1fr 1fr 1fr 1fr [end]">
	...
	<Label title="Mano de obra" --template-columns="max-content 1fr">
		<h6 class="unit">$</h6>
		<input class="precio" type="number" min="0" max="99999999" name="labor" placeholder="0" autoComplete="off" bind:value={labor} />
	</Label>
	<Label title="Repuestos" --template-columns="max-content 1fr">
		<h6 class="unit">$</h6>
		<input class="precio" type="number" min="0" max="9999999" name="cost" placeholder="0" autoComplete="off" bind:value={cost} />
	</Label>
	...
	<Label title="KM">
		<input type="number" min="0" max="9999999" name="km" placeholder="-" autoComplete="off" value={repair?.km || ''} />
	</Label>
